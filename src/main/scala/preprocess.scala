...
object preprocess{
  ...
  def main(args: Array[String]): Unit ={
    ...
    val spark = SparkSession.builder().appName("parquetGenerate").getOrCreate()
    ...
    val df = regexTokenizer.transform(
      spark.read.text("/home/cai/DM/TWENTY_NEWSGROUPS/20news-bydate-train/*")
        .withColumn("filename",input_file_name())
    ).drop('value)
    //.select(input_file_name,$"value")
    val testdf = regexTokenizer.transform(
      spark.read.text("/home/cai/DM/TWENTY_NEWSGROUPS/20news-bydate-test/*")
        .withColumn("filename", input_file_name())
    ).drop('value)
